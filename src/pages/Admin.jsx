...
export default function Admin() {
    ...
    showDialog({
      ...
      onConfirm: async () => {
        try {
          ...
          triggerUndo(
            `Đã khóa tài khoản "${user.fullName}" (${user.employeeId})`,
            async () => {
              addNotification('Khóa tài khoản', `Tài khoản của ${user.fullName} đã bị khóa.`, 'error');
            },
            async () => {
              try {
                await apiCall(`/admin/users/${user.employeeId}`, 'PUT', { isBlocked: false });
                pushLog(`Hoàn tác khóa tài khoản nhân viên: ${user.fullName} (${user.employeeId})`, 'success');
                await syncFromBackend();
              } catch (err) {
                showDialog({ title: 'Lỗi hoàn tác', message: err.message, type: 'error' });
              }
            }
          );
          await syncFromBackend();
  ...
  const handleDeleteDept = (deptName) => {
    ...
    showDialog({
      title: 'Xác nhận xoá',
      message: `Bạn có chắc chắn muốn xoá phòng ban: "${deptName}"?`,
      type: 'confirm',
      onConfirm: async () => {
        try {
          ...
          triggerUndo(
            ...
            async () => {
              try {
                await apiCall('/admin/departments', 'POST', { action: 'create', name: deptName });
  ...
  const handleDeletePos = (posName) => {
    ...
    showDialog({
      title: 'Xác nhận xoá',
      message: `Bạn có chắc chắn muốn xoá chức vụ: "${posName}"?`,
      type: 'confirm',
      onConfirm: async () => {
        try {
          ...
          triggerUndo(
            ...
            async () => {
              try {
                await apiCall('/admin/positions', 'POST', { action: 'create', name: posName });
